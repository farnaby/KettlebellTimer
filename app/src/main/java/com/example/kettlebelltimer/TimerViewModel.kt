...
class TimerViewModel(private val totalRounds: Int, context: Context) : ViewModel() {
    private var audioManager: AudioManager = KBTimerApp.getAudioManager(context)
    ...
    private fun startMainTimer(resumeAtTime : Int? = null) {
        ...
        mainTimer = object : CountDownTimer((timeForCurrentPhase * 1000).toLong(), 1000) {
            ...
            override fun onFinish() {
                finishCurrentExerciseOrRest()
            }
        }.start()
    }

    private fun finishCurrentExerciseOrRest() {
        when (_currentPhase.value) {
            WorkoutPhase.EXERCISE -> {
                ...
                if (currentExercise < EXERCISES_PER_ROUND) {
                    ...
                    totalSecondsElapsed = calculateTotalWorkoutDuration()
    ...
    private fun updateProgressValues() {
        if(_currentPhase.value == WorkoutPhase.INITIAL_COUNTDOWN) {
            _currentPhaseProgress.value = 0f
            _totalWorkoutProgress.value = 0f
            return
        }
